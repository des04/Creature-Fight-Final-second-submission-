...
public class Golem extends Creature
{
    ...
    public void act() 
    {
        ...
        if( getHealthBar().getCurrent() <= 0 )
        {
            ...
            if ( playerWorld.getNewOneCreature(0).getHealthBar().getCurrent() > 0 )
            {
                ...
                //TODO (71): Clear the text (using an empty String, "") at the location that it stated Golem had fainted
            ...
            else if ( playerWorld.getNewOneCreature(2).getHealthBar().getCurrent() > 0 )
            {
                ...
                playerWorld.setTurnNumber(1);
                //TODO (76): Clear the text (using an empty String, "") at the location that it stated Golem had fainted
                getWorld().showText("", getWorld().getWidth()/2, getWorld().getHeight()/2 + 26);
    ...
    public void attack(int idx)
    {
        ...
        else
        {
            ...
            else if (enemyType.equalsIgnoreCase("Flying"))
            {
                ...
                getWorld().showText("The attack wasn't effective.", getWorld().getWidth()/2, getWorld().getHeight()/2+26 );
                Greenfoot.delay(30);
            }
            else
            {
    ...
    public void switchCreature( int idx )
    {
        ...
        if (getHealthBar().getCurrent() <= 0 )
        {
            JOptionPane.showMessageDialog(null, "The creature you have chosen to switch to has fainted, chose another one." );
        }
        else
        {
            while ( getX() > 0  )
